...
const MOUSE_RADIANS_PER_PIXEL = PI / 1800.0
const STICK_RATE = Vector2(150.0, 100.0) * PI / 180.0
const LOOK_IDLE_SECONDS = 0.8
const RETURN_TAU = 0.35
...
const FIRST_PERSON_PITCH = -10.0 * PI / 180.0
...
var look_idle: float = LOOK_IDLE_SECONDS
...
func has_manual_look() -> bool:
	return absf(look_yaw) + absf(look_pitch) > 0.001 or look_idle < LOOK_IDLE_SECONDS

static func speed_factor(kmh: float) -> float:
...
func _update_look(dt: float, kmh: float, enabled: bool, summit: bool) -> void:
	if not enabled:
		clear_look_input()
		return
	var delta = pending_mouse * MOUSE_RADIANS_PER_PIXEL + pending_stick * STICK_RATE * dt
	pending_mouse = Vector2.ZERO
	pending_stick = Vector2.ZERO
	if not delta.is_zero_approx():
		...
		look_idle += dt
		var return_dt = dt if recentering else maxf(0.0, look_idle - maxf(LOOK_IDLE_SECONDS, previous_idle))
		if recentering or (kmh > 5.0 and not summit):
			var retain = exp(-return_dt / RETURN_TAU)
			look_yaw *= retain
			look_pitch *= retain
...
func update_camera(sim, field, rider_position: Vector3, dt: float, menu: bool = false, active: bool = true, summit: bool = false) -> void:
	var speed: float = sim.velocity.length()
	var kmh: float = sim.speed_kmh()
	...
	var target_speed = speed_factor(kmh) if effects_enabled and not menu and not summit else 0.0
	speed_blend = lerpf(speed_blend, target_speed, 1.0 - exp(-2.5 * dt)) if initialized else target_speed
	...
	var rest_fov = settings.rest_fov if riding else BASE_FOV
	var fast_fov = settings.fast_fov if riding else FAST_FOV
	fov = lerpf(rest_fov, fast_fov, clampf(speed_blend, 0.0, 1.0))
	...
	boom_distance = maxf(0.4, lerpf(settings.rest_distance, settings.fast_distance, speed_blend) - carve_blend * 0.6)
	var height_offset = carve_blend * 0.25 + (sim.effective_tuck * 0.25 if animated else 0.0) + compression
	boom_height = maxf(1.0, lerpf(settings.rest_height, settings.fast_height, speed_blend) - height_offset)
	if sim.crashed:
	...
	var boom_blend = 1.0 - exp(-sim.tuning.camera_response * dt)
	if initialized:
	...
	var tau = MAX_SMOOTHING_TAU * settings.vertical_smoothing / 100.0
	...
	if mode >= 0:
		# Optical pitch depends on nominal framing, never on displaced camera or
		# rider heights, terrain look-ahead, contact switches, tuck or compression.
		# Short booms need a nearer nominal aim to keep the skier in the frame.
		var nominal_distance = lerpf(settings.rest_distance, settings.fast_distance, speed_blend)
		var nominal_height = lerpf(settings.rest_height, settings.fast_height, speed_blend)
		var anticipation = minf(nominal_distance, lerpf(4.0, 6.0, speed_blend))
		var pitch = FIRST_PERSON_PITCH if close_view else -atan2(nominal_height - 0.6, nominal_distance + anticipation)
		pitch += deg_to_rad(settings.first_person_pitch_offset if close_view else settings.chase_pitch_offset)
