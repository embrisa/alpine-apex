...
const KEYS = ["quality", "display_mode", "upscaler", "render_scale", "fps_limit", "terrain_gi"]
var quality: int = 2
var display_mode: String = "fullscreen"
var upscaler: String = "fsr2"
...
func restore(values: Dictionary) -> void:
	...
	if upscaler not in ["native","fsr2"]: upscaler = "fsr2"
...
func apply_arguments(args: PackedStringArray) -> void:
	for arg in args:
		...
		elif arg.begins_with("--upscaler=") and value in ["native","fsr2"]: upscaler = value
...
func apply_viewport(viewport: Viewport) -> void:
	...
	viewport.scaling_3d_mode = Viewport.SCALING_3D_MODE_FSR2 if upscaler=="fsr2" else Viewport.SCALING_3D_MODE_BILINEAR
	viewport.scaling_3d_scale = render_scale if upscaler=="fsr2" else 1.0
	viewport.msaa_3d = Viewport.MSAA_DISABLED if upscaler=="fsr2" else Viewport.MSAA_2X
...
func report(viewport: Viewport, output_pixels: Vector2i) -> Dictionary:
	...
	return {"preferences":snapshot(),"output_pixels":[output_pixels.x,output_pixels.y],
		...
		"viewport_scale":scale_value,"msaa":viewport.msaa_3d,"fps_limit":Engine.max_fps}
