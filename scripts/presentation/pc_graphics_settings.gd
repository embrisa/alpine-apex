...
## stock renderers, and to spatial bilinear on Metal. Fable's M4 configuration
## comparison is recorded in docs/MAC_PERFORMANCE_BASELINE.json.
...
func is_temporal() -> bool:
	return effective_upscaler() not in SPATIAL_UPSCALERS or frame_generation_enabled()

func frame_generation_enabled() -> bool:
	# The DX12 generation hook requires the temporal pass. Keep explicit
	# bilinear spatial, retaining the preference for a later compatible mode.
	return frame_generation and has_native_fsr() and effective_upscaler()!="bilinear"
...
func apply_viewport(viewport: Viewport) -> void:
	...
	if has_native_fsr(): Engine.get_singleton("AlpineFidelityFX").set_options(upscaler,frame_generation_enabled())
...
func apply_display(window: Window, requested_pixels: Vector2i = Vector2i.ZERO) -> void:
	display.apply(window,requested_pixels,frame_generation_enabled())
