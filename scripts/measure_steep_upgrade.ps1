param([switch]$Baseline,[switch]$Timing,[switch]$Extended,[switch]$Handover,[switch]$Supplemental)
$ErrorActionPreference = 'Stop'
$alpineRoot = Split-Path $PSScriptRoot -Parent
$alpineLabel = $(if ($Baseline) {'before'} else {'after'}) + $(if ($Timing) {'_timing'} else {'_visual'})
if ($Handover) { $alpineLabel += '_handover' }
if ($Supplemental) { $alpineLabel += '_supplemental' }
$alpineOutput = Join-Path $alpineRoot "artifacts/steep_animation_physics_upgrade/$alpineLabel"
...
$alpineArgs = @('--path',('"'+$alpineRoot+'"'),'--script','tests/steep_upgrade_playtest.gd','--','--ui-staged-loading','--graphics-quality=high','--terrain-gi=off')
if ($Baseline) { $alpineArgs += '--baseline-v16' }
if ($Timing) { $alpineArgs += '--timing' }
if ($Extended) { $alpineArgs += '--extended' }
if ($Handover) { $alpineArgs += '--handover' }
if ($Supplemental) { $alpineArgs += '--supplemental' }
function Get-RefinementHashes {
    $alpineHashMap = @{}
    $alpineFolders = @('scripts/core','scripts/presentation','scripts/world','config','assets')
    if ($Baseline) { $alpineFolders += 'artifacts/steep_animation_physics_upgrade/reference' }
    ...
    foreach ($alpineFile in @('scripts/main.gd','tests/steep_upgrade_playtest.gd','scripts/measure_steep_upgrade.ps1','project.godot','main.tscn')) {
...
while (-not $alpineProcess.HasExited) {
    ...
    $alpineOther = @(Get-Process -Name Godot*,blender*,ffmpeg* -ErrorAction SilentlyContinue | Where-Object { $_.Id -ne $alpineProcess.Id -and $_.Id -ne $alpineWorker.Id -and $_.ProcessName -notlike '*console*' } | Select-Object Id,CPU,WorkingSet64)
