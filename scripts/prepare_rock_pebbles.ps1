...
try {
    if ($Mode -eq 'Build') {
        ...
        $pebblePreview = Join-Path $pebbleRoot 'artifacts/rock_pebble_scale_20260914/native_project'
...
'@ | Set-Content -LiteralPath (Join-Path $pebblePreview 'project.godot') -Encoding utf8
        ...
        $pebbleArgs = @('--path',$pebblePreview,'--script','preview.gd','--',
            ...
            ('--output='+(Join-Path $pebbleRoot 'artifacts/rock_pebble_scale_20260914/native')))
        & ./scripts/run_guarded.ps1 -FilePath $pebbleEngine -Arguments $pebbleArgs -Label cosmetic-gravel-scale-preview -WorkloadMode Shared -TimeoutSeconds 120
    }
