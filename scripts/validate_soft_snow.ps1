...
if (-not $Worker) {
    ...
    & "$PSScriptRoot/run_guarded.ps1" -FilePath pwsh -Arguments @('-NoProfile','-File',$PSCommandPath,'-Worker','-Stage',$Stage) -Label "soft-snow-$Stage" -TimeoutSeconds 1800 -CollectGpuMemory
    exit $LASTEXITCODE
}
if ($Stage -eq 'checks') {
