...
func run() -> void:
	...
	if destination.is_empty():
		push_error("ALPINE_BAKE_OUTPUT must name the staged data/default_mountain_v18.physical")
		quit(1)
		return
	...
	if not prepared.load_cached(field,preload("res://scripts/presentation/graphics_quality.gd").preset(2),Cache.Job.new()):
		...
		quit(1); return
	var scenery_destination = destination.get_base_dir().path_join("default_mountain_v18.scenery")
