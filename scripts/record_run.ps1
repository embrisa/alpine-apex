...
try {
    if ($Replay) {
        & ./scripts/run_guarded.ps1 -FilePath pwsh -Arguments @('-NoProfile','-File','scripts/benchmark_pc.ps1','-Label',$Label,'-InputTrace',$Replay,'-ScenarioReplay','-Repetitions','1') -Label $Label -TimeoutSeconds 1200 -CollectGpuMemory
    } else {
        ...
        & ./scripts/run_guarded.ps1 -FilePath ./godotw.ps1 -Arguments $alpineArgs -Label $Label -TimeoutSeconds 3600
    }
    exit $LASTEXITCODE
