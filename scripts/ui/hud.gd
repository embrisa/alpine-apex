...
func _sync_fidelityfx_choices() -> void:
	...
	display_controls.upscaler.tooltip_text = "Auto selects the best provider reported by AMD's SDK." if native_fsr else ("Auto uses spatial bilinear upscaling on Metal, where this engine's FSR 2 pass costs about 15 ms per frame." if settings.resolve_upscaler("auto",false,RenderingServer.get_current_rendering_driver_name())=="bilinear" else "Auto uses FSR 2 in this engine.")
