...
func _draw() -> void:
	...
	draw_polyline(points,Color("007fa7"),2.0,true)
	...
	draw_line(Vector2(point.x,plot.position.y),Vector2(point.x,plot.end.y),Color("506778"))
	...
	for speed in [0,100,200,300]:
		...
		draw_string(ThemeDB.fallback_font,Vector2(clampf(x-12,0,size.x-28),size.y-3),caption,HORIZONTAL_ALIGNMENT_LEFT,-1,12,Color("506778"))
