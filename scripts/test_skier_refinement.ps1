param([string[]]$Suites = @('physics_suite','runtime_suite','downhill_control_suite','downhill_contact_suite','handling_suite','handling_upgrade_suite','high_speed_turns_suite','high_speed_balance_suite','jump_suite','airborne_control_suite','airborne_pose_suite','ski_attachment_suite','turn_anatomy_suite','impact_recovery_suite','rock_terrain_suite','competitive_suite','rider_lifecycle_suite','skier_motion_suite','skier_animation_suite','skier_refinement_suite'))
$ErrorActionPreference = 'Stop'
$alpineRoot = Split-Path $PSScriptRoot -Parent
$alpineOutput = Join-Path $alpineRoot 'artifacts/skier_refinement_v16'
New-Item -ItemType Directory -Force $alpineOutput | Out-Null
$alpineResultPath = Join-Path $alpineOutput 'suites.json'
$alpineResults = @()
...
foreach ($alpineSuite in $Suites) {
    ...
    Write-Output "REFINEMENT_TEST_START $alpineSuite"
    ...
    Write-Output "REFINEMENT_TEST_END $alpineSuite exit=$alpineExit problems=$($alpineProblems.Count)"
}
