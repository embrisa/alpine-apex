...
func configure(source, field) -> void:
	...
	for band in 3:
		...
		for i in count:
			...
			var center = apron_center+radial*([7400.0,11700.0,16000.0][band]+rng.randf_range(-650,650))
...
func _height_uncached(p: Vector2) -> float:
	...
	var distance_m = p.distance_to(p.clamp(physical_bounds.position,physical_bounds.end))
	var blend = smoothstep(COLLAR_M,2300.0+noise.get_noise_2d(p.x+740,p.y-320)*900.0,distance_m)
...
func sample_at(p: Vector2) -> Dictionary:
	...
	var snow = clampf(smoothstep(snowline-650,snowline+450,h)+hollow*.17,0,1)
