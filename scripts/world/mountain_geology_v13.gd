...
func _add_technical_regions(field) -> void:
	...
	for face in field.faces:
		...
		for attempt in 18000:
			if placements.size()-start>=2450: break
...
func _protected(field, p: Vector2, radius: float) -> bool:
	...
	for face in field.adjacent_faces(p):
		...
		# Only local band openings and the narrowest tributary floors reserve
		# clearance. Their finite extents never create a full-mountain corridor.
		for offset in [Vector2.ZERO,Vector2(radius,0),Vector2(-radius,0),Vector2(0,radius),Vector2(0,-radius)]:
			var at=q+offset
			if face.snow_gap(at.x,at.y)>.35: return true
		if face.channel_values(q.x,q.y).y>.85: return true
	return false
