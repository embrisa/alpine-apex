...
const DIRECTORY = "user://mountain_bake_cache_v10"
static func generate(seed_value: int):
	var begin = Time.get_ticks_usec()
	var key = "%d|%s" % [seed_value,Engine.get_version_info().string]
	for path in ["res://scripts/world/generators/alpine_massif_v10.gd","res://scripts/world/generators/alpine_face_v10.gd","res://scripts/world/heightfield_surface.gd"]:
	...
	if DirAccess.make_dir_recursive_absolute(DIRECTORY)==OK:
		...
		if file:
			var payload = {"key":key,"heights":field.heights,"obstacles":field.obstacles,"exposure":field.exposure_image.get_data(),"height_sha256":field.height_checksum,"obstacle_sha256":field.obstacle_checksum,"bake_ms":field.generation_ms}
...
static func _valid(data: Variant,key: String) -> bool:
	if not data is Dictionary or data.get("key")!=key or data.size()!=8: return false
