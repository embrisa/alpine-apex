...
func _init(mountain_seed: int = 849205174) -> void:
	...
	rng.seed = seed_value
	parameters = {"pitch":rng.randf_range(.34,.44), "phase":rng.randf_range(-PI,PI),
		"bend":rng.randf_range(18,45), "divide":rng.randf_range(-45,45),
		"ridge_height":rng.randf_range(40,65), "ridge_width":rng.randf_range(46,72),
		"bowl_z":rng.randf_range(440,620), "bowl_depth":rng.randf_range(22,42),
		...
		"treeline_z":rng.randf_range(650,850), "peak_height":rng.randf_range(110,180)}
	...
	_scatter(rng)
	features = [
		{"name":"Summit shoulder", "position":Vector2(0,25)},
		...
		{"name":"Forest & snowfields", "position":Vector2(-150,1250)},
		{"name":"Lower basin", "position":Vector2(0,1700)}]
...
func _landform(x: float, z: float) -> float:
	...
	var base = 2220.0-z*.18-(parameters.pitch-.18)*_ramp(z,80,280)
	base += (parameters.pitch-.13)*_ramp(z,1370,270)
	...
	var branch = 26.0*_gauss(x-branch_x,45)*_gauss(z-1000,300)
	...
	var rollers = 5.0*_gauss(z-parameters.roller_z,76)*_gauss(cross-90,115)
	# Detail is subordinate to the macroforms and slow along the descent.
	var detail = noise.get_noise_2d(x,z*.55)*2.0+sin(x*.08+z*.009)*.45
	var apron = smoothstep(80,230,z)
...
func _scatter(rng: RandomNumberGenerator) -> void:
	for i in range(1850):
		...
		var tree = rng.randf()<forest*patch*.85 and normal.y>.79
		if not tree and rng.randf()>.20: continue
