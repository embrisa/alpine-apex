...
func prepare(field) -> void:
	...
	rng.seed = field.seed_value ^ 0x47454F11
...
func finish(field) -> void:
	...
	for placed in placements:
		...
		for local in record.seating:
			...
			shift=minf(shift,float(field.sample(p.x,p.z).height)-p.y-.08)
	...
	var rng=RandomNumberGenerator.new(); rng.seed=field.seed_value ^ 0x7A10511
	for source in sources:
		...
		var count=22 if ice else (70 if record.category=="cliffs" else 28)
		for i in count:
	...
	# Depositional stones belong lower down, sparsely outside clear glade floors.
	for face in field.faces:
		for i in 240:
			var z=rng.randf_range(1700,2620)
			var side=-1 if rng.randf()<.5 else 1
			var x: float=face.glade_x(z,side)+side*rng.randf_range(28,180)
			_place(field,face,catalog.choose("medium" if i%5==0 else "small","rounded",rng),Vector2(x,z),rng,.28)
	...
	var shoulder_rng=RandomNumberGenerator.new(); shoulder_rng.seed=field.seed_value ^ 0xBED110
	for source in sources:
...
func _place(field, face, asset: String, q: Vector2, rng: RandomNumberGenerator, burial: float) -> Dictionary:
	...
	for local in record.seating:
		...
		var required: float=field.sample(v.x,v.z).height-v.y-.10
		low=minf(low,required); offsets.append(required)
	if offsets.is_empty(): return _reject()
	offsets.sort()
	var h: float=field.sample(p.x,p.y).height-record.size_m.y*scale_value*burial
...
func _visible(field, pose: Transform3D, record: Dictionary) -> bool:
	...
	return top.y-float(field.sample(top.x,top.z).height)>maxf(.05,record.size_m.y*.35)
...
func _protected(field, p: Vector2, radius: float) -> bool:
	...
	for face in field.adjacent_faces(p):
		...
		for side in [-1,1]:
			var lane: float=face.gully_x(q.y,side) if q.y<1850 else face.glade_x(q.y,side)
			if absf(q.x-lane)<radius+18: return true
		var drop: Dictionary=face.landforms[4]
		if absf(q.x-drop.position.x)<radius+drop.width+45 and q.y>drop.position.y-drop.approach_length-radius-70 and q.y<drop.position.y+drop.landing_length+radius+80: return true
		# Test the footprint edges as well as the centre against snow ramps.
		for offset in [Vector2.ZERO,Vector2(radius,0),Vector2(-radius,0),Vector2(0,radius),Vector2(0,-radius)]:
			if face.snow_gap(q.x+offset.x,q.y+offset.y)>.15: return true
	return false
...
func _stamp_foundations(field) -> void:
	...
	for placed in placements:
		...
		for local in record.seating:
			var p: Vector3=placed.pose*local
			var lift: float=clampf(p.y+.12-field.sample(p.x,p.z).height,0,STAMP_LIMIT_M)
			if lift<=0: continue
...
func _reindex() -> void:
	...
	for placed in placements: collision.add(placed,catalog.records[placed.asset])
...
func fingerprint() -> String:
	var values: Array=[VERSION,catalog.fingerprint]
	for placed in placements:
		var pose: Transform3D=placed.pose
		values.append([placed.id,placed.asset,pose.origin.x,pose.origin.y,pose.origin.z,
			pose.basis.x.x,pose.basis.x.y,pose.basis.x.z,pose.basis.y.x,pose.basis.y.y,pose.basis.y.z,
			pose.basis.z.x,pose.basis.z.y,pose.basis.z.z,placed.solid])
