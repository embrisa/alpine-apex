...
keep labels, bars and dial together. HUD backgrounds default **off**; enable
**Settings → Interface & HUD → HUD backgrounds** for optional instrument panels.
Text retains its normal weight with an 11-pixel logical minimum and a faint
one-pixel shadow. Text and gauges have no dark outlines or edging. The user
rejected the heavier outlined treatment; keep the HUD restrained. Riding has
no screen-edge shading. The debug panel follows the same preference. Transient frames disappear with
their notice. Menu navigation, loading, errors, world markers and full-screen
impact effects are separate.
...
.85–1.4 requested UI scale, 0–8% safe area and the HUD-background boolean.
Effective scale is bounded to keep a minimum usable logical canvas at small
resolutions; UI still renders at output pixels. Resolution changes recompute
positions and clamp complete widget bounds.
...
hide riding instruments. Reload handoffs retain the chosen layout, UI scale and
background preference. The HUD editor previews the same background setting.
...
## HUD background follow-up (2026-09-11)

The user's background-free HUD preference supersedes the panel appearance in
older interface captures. The initial heavy outlines were rejected and removed
from both text and gauges, together with synthetic font emboldening. Only a
faint one-pixel text shadow remains. The revised layout suite passes 95/95,
including both saved boolean values, malformed input, live/editor synchronization
and unchanged per-widget layout (`hud-backgrounds-soft-layout`). The toggle's
initial interaction regression passed 40/40
(`hud-backgrounds-final-interface_overhaul_suite`).

`hud-backgrounds-soft-native` passed with no engine errors on custom DX12 /
RX 9070. Inspected eleven native captures under `artifacts/hud_backgrounds/`:
actual 1280×720 and 3840×2160 output, frozen default v15 Standard mountain views,
solid white/dark contrast fixtures, night, optional panels, background-free
telemetry, settings and HUD editor. The real menu router enabled and disabled
the preference via injected controller A events. Sample HUD values are for
readability review. This follow-up does not establish new performance numbers,
physical-controller acceptance or full-descent readability acceptance.
