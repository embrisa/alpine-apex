...
on Metal after Fable's measured configuration comparison; see
...
`open -n -W -a Godot.app --args ...` yourself when they must render. The first
local import is large and can take several minutes.
...
```powershell
$env:ALPINE_BAKE_OUTPUT = 'C:\absolute\new-build\data\default_mountain_v17.physical'
