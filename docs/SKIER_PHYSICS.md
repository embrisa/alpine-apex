...
advance it, create support or steer the rider. Read [architecture](ARCHITECTURE.md)
...
[animation ownership](SKIER_ANIMATION.md) and [anatomical fitting](SKIER_ANATOMY.md).
...
contracts. [Geology](GEOLOGY_V11.md) covers solid mineral proxies. Equipment
follows the physical handoff; it cannot create normal-skiing force authority.
See [skier assets](SKIER.md) and [equipment](EQUIPMENT.md) for editable sources.
...
performance. [Validation](VALIDATION.md) records the current acceptance boundaries.
