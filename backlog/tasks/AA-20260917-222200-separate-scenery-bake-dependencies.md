...
GenerationSources.signature(true) includes foliage_sight.gd and foliage_sight.gdshaderinc although neither creates serialized scenery. Broad consumers remain coupled and must stay covered.
...
- [ ] Prove render-only changes preserve the cache key/read; real placement, assets, terrain, bounds, bake/schema changes reject stale data. Check compact cache integrity/export paths and bounded warm reuse. No steady-state FPS claim.
