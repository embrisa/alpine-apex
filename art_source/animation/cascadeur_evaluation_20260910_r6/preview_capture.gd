extends SceneTree
const FOLDER = "res://art_source/animation/cascadeur_evaluation_20260910_r6/"
const REV = "res://artifacts/pose_review/revisions/cascadeur-20260910-r6-source/"
...
func run():
	...
	var paths=[FOLDER+"ready_compression_r6_animation.glb",FOLDER+"baked_animation_audit.json",FOLDER+"preview_capture.gd"]
	...
	assert(document.append_from_file(FOLDER+"ready_compression_r6_animation.glb",state)==OK)
	...
	write(REV+"capture/cascadeur_grounded.json",{"scope":report.scope,"rig":rig,"frames":rows})
	write(REV+"capture/manifest.json",{"version":1,"capture_fps":30,"preview_only":true,"scope":report.scope,"tick_policy":"Authored frame index times four for review tooling; no physics ticks were executed.","stable_sources":sources==after,"sources":sources,"sources_after":after,"failures":[] if passed else ["Import/writer/scale failed"],"scenarios":[{"name":"cascadeur_grounded","frames":rows.size()}],"engine":report.engine,"engine_sha256":report.engine_sha256})
	write(REV+"selection.json",{"cascadeur_grounded":[0,12,24,30,44,60]})
	write(REV+"selection_reasons.json",{"0":"Athletic ready entry","12":"End of ready hold","24":"Approach to compression","30":"Maximum authored compression","44":"Recovery","60":"Settled ready exit","scope":"Authored beats, not detected physical events"})
