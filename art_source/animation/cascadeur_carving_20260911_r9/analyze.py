"""Compare fitted poses and rigid attachments at matched inputs in separate simulations."""
...
report={'scope':'726 matched input times on model 28, 22 cm snow. R8 pressure support versus stronger R9 support; R7 hands in both. Separate physics, geometry and user preference.','scenarios':{}}
for case in read(base/'cascadeur-20260911-r9-02-after/capture/manifest.json')['scenarios']:
    name=case['name'];a=read(base/'cascadeur-20260911-r9-02-before/capture'/f'{name}.json');b=read(base/'cascadeur-20260911-r9-02-after/capture'/f'{name}.json')
    rig=b['rig'];names=[r['name'] for r in rig];ids={n:i for i,n in enumerate(names)};rest={r['name']:np.array(r['rest']['origin']) for r in rig}
    metrics={'input_or_tick_mismatches':0,'max_final_joint_difference_m':0.,'max_binding_error_m':0.,'max_grip_error_m':0.,'max_segment_error_m':0.,'min_cuff_forward_deg':1000.,'max_cuff_forward_deg':-1000.,'max_cuff_side_deg':0.,'max_applied_amount':0.,'frames':len(b['frames']),'max_rotation_step_deg':0.}
    previous=None;diffs=[]
    for old,row in zip(a['frames'],b['frames']):
        for key in ['tick','input']:
            metrics['input_or_tick_mismatches']+=int(old[key]!=row[key])
    ...
    metrics['passed']=metrics['input_or_tick_mismatches']==0 and max(metrics[k] for k in ['max_binding_error_m','max_grip_error_m','max_segment_error_m'])<.0001 and -.1<=metrics['min_cuff_forward_deg'] and metrics['max_cuff_forward_deg']<=24.1 and metrics['max_cuff_side_deg']<=10.1
