...
const Motion=preload("res://tests/cascadeur_r7_playtest/live_motion.gd")
const Equipment=preload("res://scripts/presentation/skier_equipment.gd")
const BASE="res://art_source/animation/cascadeur_carving_20260910_r7/"
const REV="res://artifacts/pose_review/revisions/cascadeur-20260910-r7-"
const DT=1.0/120.0
const CASES=["straight","steering_left","steering_right","carve_reversal","carve_taps","tuck_turn"]
var visuals=[]
var import_report={}
var rigs=[]
var failures=[]
...
func hashes():
	...
	for name in ["gameplay_capture.gd","build_controls.py","carving_r7_animation.glb"]:h[BASE+name]=FileAccess.get_sha256(BASE+name)
	for file in ["candidate_asset.gd","live_motion.gd"]:
...
func load_candidate():
	...
	assert(doc.append_from_file(BASE+"carving_r7_animation.glb",state)==OK)
...
func row_for(visual,sim,name,frame,t):
	...
	var source=landmarks(full.sample_raw(full.candidate,.5 if full.diagnostics.get("physical_turn",0)>0 else 1.5))
...
func run():
	...
	for enabled in [false,true]:
		...
		visual.animation.full_motion=Motion.new();visual.animation.full_motion.candidate_enabled=enabled
	await process_frame
	...
	for name in CASES:
		...
		for frame in 121:
			if frame>0:
				for sub in 4:
					...
					for visual in visuals:
						visual.step_animation(DT,sim,input,plane)
		...
		reasons[name]={"0":"Warm neutral","15":"Light steer / entry","27":"Light hold","42":"Strong steer / reversal","54":"Loaded turn","75":"Direction or tuck transition","93":"Steer release","108":"Residual turn","120":"Recovery"}
