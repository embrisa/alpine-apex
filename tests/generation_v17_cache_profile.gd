...
func run() -> void:
	...
	preload("res://tests/test_report.gd").write("res://artifacts/generation_v17/v17_cache_wall_baseline.json",JSON.stringify(result,"\t"))
	print("V17_FULL_CACHE_PROFILE ",JSON.stringify(result)); quit(1 if failed else 0)
