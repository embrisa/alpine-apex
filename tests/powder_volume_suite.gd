...
func run() -> void:
	...
	preload("res://tests/test_report.gd").write(OUTPUT+"/technical-showcase-v9.apexmountain",recipe.share_text())
	preload("res://tests/test_report.gd").write(OUTPUT+"/physical_results.json",JSON.stringify({"checks":checks,"failures":failures,"metrics":metrics},"\t"))
