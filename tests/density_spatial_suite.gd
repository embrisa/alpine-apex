...
func run() -> void:
	...
	check(Definition.CURRENT_VERSION==16 and Definition.parse_seed("42").version==16,"Bare seeds use current v16")
	check(Definition.CurrentTerrain.GENERATOR_VERSION==16 and not Definition.CurrentTerrain.GENERATOR_ID.is_empty(),"Versioned fields expose scenery dispatch metadata")
