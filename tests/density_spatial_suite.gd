...
func run() -> void:
	...
	check(Definition.CURRENT_VERSION==17 and Definition.parse_seed("42").version==17,"Bare seeds use current v17")
	check(Definition.CurrentTerrain.GENERATOR_VERSION==17 and not Definition.CurrentTerrain.GENERATOR_ID.is_empty(),"Versioned fields expose scenery dispatch metadata")
