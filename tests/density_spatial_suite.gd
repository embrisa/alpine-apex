...
func run() -> void:
	...
	check(Definition.CURRENT_VERSION==15 and Definition.parse_seed("42").version==15,"Bare seeds use current v15")
	check(Definition.CurrentTerrain.GENERATOR_VERSION==15 and not Definition.CurrentTerrain.GENERATOR_ID.is_empty(),"Versioned fields expose scenery dispatch metadata")
