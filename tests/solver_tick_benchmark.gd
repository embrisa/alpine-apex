extends SceneTree
## Headless paired solver timing: exact per-tick state hash plus wall time for
## a scripted descent on the standard mountain. Compare two checkouts by hash
## equality first; timing is a same-machine, same-process-count observation.
