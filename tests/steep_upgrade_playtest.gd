...
## Matched v12 scripted motion clips or a separate screenshot-free 4K run.
...
func run():
	...
	before = "--baseline-v16" in OS.get_cmdline_user_args()
	timing = "--timing" in OS.get_cmdline_user_args()
	folder_label = ("before" if before else "after")+("_timing" if timing else "_visual")
	if "--handover" in OS.get_cmdline_user_args(): folder_label += "_handover"
	if "--supplemental" in OS.get_cmdline_user_args(): folder_label += "_supplemental"
	output = "res://artifacts/steep_animation_physics_upgrade/"+folder_label
	DirAccess.make_dir_recursive_absolute(output)
	requested = Vector2i(3840,2160) if timing else Vector2i(960,720)
	field = Definition.generate(849205174,12)
	set_meta("mountain_to_load",{"definition":Definition.from_field(field,"Skier refinement"),"field":field})
	var scene = "res://artifacts/steep_animation_physics_upgrade/reference/main.tscn" if before else "res://main.tscn"
	if not FileAccess.file_exists(scene): printerr("Missing frozen v16 reference: ",scene); quit(2); return
	...
	if "--extended" in OS.get_cmdline_user_args():
		scenarios.append_array([
			...
			{"name":"small_correction","steer":.2,"speed_kmh":60.0,"tuck":0.0,"seconds":4.0},
			{"name":"major_turn","steer":1.0,"speed_kmh":60.0,"tuck":0.0,"sustain":true,"seconds":8.0},
			{"name":"switch_turn","steer":-.7,"speed_kmh":90.0,"switch":true,"seconds":4.0},
			{"name":"natural_departure","height":.25,"up":0.0,"seconds":2.0},
			{"name":"assist_off","height":16.0,"up":5.0,"assist":false,"seconds":5.0},
			{"name":"assist_gradual","height":16.0,"up":5.0,"assist":true,"seconds":5.0},
			{"name":"near_landing_override","height":12.0,"up":4.0,"assist":true,"override":true,"seconds":5.0},
			{"name":"spin_release","height":28.0,"up":8.0,"trick":"spin","seconds":6.0},
			{"name":"flip_release","height":28.0,"up":8.0,"trick":"flip","seconds":6.0},
			{"name":"switch_flip","height":28.0,"up":8.0,"trick":"flip","switch":true,"seconds":6.0},
			{"name":"safety_grab","height":16.0,"up":6.0,"trick":"grab","seconds":5.0},
			{"name":"one_ski_landing","height":2.0,"up":-3.0,"roll":.15,"seconds":3.0},
			{"name":"low_reserve_recovery","steer":.2,"speed_kmh":50.0,"reserve":.25,"seconds":4.0},
			{"name":"exhaustion_crash","height":7.0,"up":-8.0,"reserve":.08,"expect_crash":true,"seconds":4.0}])
	if "--handover" in OS.get_cmdline_user_args():
		scenarios = [{"name":"near_landing_override","height":12.0,"up":4.0,"assist":true,"override":true,"seconds":3.0}]
	if "--supplemental" in OS.get_cmdline_user_args():
		scenarios = [
			{"name":"tuck_to_turn","steer":1.0,"turn_start":1.0,"sustain":true,"tuck":1.0,"speed_kmh":90.0,"seconds":4.0},
			{"name":"skid_to_carve","steer":.65,"turn_start":.6,"sustain":true,"tuck":0.0,"speed_kmh":60.0,"slip":.45,"seconds":4.0},
			{"name":"exhaustion_crash","height":7.0,"up":-8.0,"reserve":.08,"expect_crash":true,"seconds":4.0}]
	for request in scenarios: await ride(request)
	measuring = false
	var report = {"physics":game.sim.MODEL_VERSION,"seed":849205174,"version":12,"unranked":not game.session.eligible,"cases":evidence,"failures":failures,
...
func ride(request: Dictionary):
	...
	else: game.sim.tuning.landing_assist_enabled = before
