...
func run(parent, tasks: Array[String]) -> Dictionary:
	...
	evidence.loading_estimate_telemetry = {"path":"user://generation_measurements_v15.json","before":loading_estimate_before,"after":FileAccess.get_sha256("user://generation_measurements_v15.json"),"owner":"GenerationEstimates records normal loading diagnostics; excluded from personal preferences, races, replays and records"}
...
func persistent_files() -> Dictionary:
	...
	for file in DirAccess.get_files_at("user://"):
		...
		if file=="generation_measurements_v15.json": continue
