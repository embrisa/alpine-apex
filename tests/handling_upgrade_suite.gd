...
func _recorded_requests() -> void:
	...
	check(identity.physics==32 and Replay.VERSION==7 and Replay.INPUT_WIDTH==9 and recording.inputs.size()==150*Replay.INPUT_WIDTH,"Model 32/replay 7 retain all nine fields including held preparation")
	...
	incompatible.compatibility.physics = 31
	check(Replay.decode(incompatible,identity,recording.duration)==null,"Previous physics model cannot load as a compatible model 32 ghost")
