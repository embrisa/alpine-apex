...
func run() -> void:
	...
	check(game.session.course_id==game.session.laboratory_identity() and game.session.course_id.contains("physics-v30-"),"Current physics and crash recovery use the laboratory benchmark identity")
	key(KEY_F2)
