...
func run() -> void:
	...
	preload("res://tests/test_report.gd").write(output,JSON.stringify(plain({"checks":checks,"failures":failures,"cases":cases,"scope":"Actual final-pose analytic fixtures; consecutive 60 Hz joint deltas exclude the initial unstepped reset pose. Not native/clothing/controller acceptance."}),"\t"))
