...
const Terrain = preload("res://scripts/world/generators/drainage_v1.gd")
...
func run() -> void:
	...
	for side in sides:
		...
		# Pin this test entry at heading 0 so the measured route stays comparable.
		# Normal free-ski entry uses field.spawn_heading(), tested in the seed suite.
		...
		for i in range(24000):
			...
			input.brake = clampf((sim.speed_kmh()-58)/10,0,1)
			input.tuck = 0.0
			if rendered:
				game.intent = input
			...
			if i%1200==0:
				sample_points.append([sim.position.x,sim.position.y,sim.position.z])
				if rendered and capture_enabled:
					await RenderingServer.frame_post_draw
					root.get_texture().get_image().save_png("res://artifacts/mountain_generation/descent_%d.png" % i)
			if sim.crashed or sim.position.z>=1740: break
		recording = false
		results.append({"side":"west" if side<0 else "east","finished":sim.position.z>=1740,"crashed":sim.crashed,"reason":sim.crash_reason,
	...
	var output = {"results":results,"rendered":rendered,"actual_pixels":[actual_pixels.x,actual_pixels.y],"capture_overhead_included":capture_enabled,
	...
	var path = "res://artifacts/mountain_generation/descent_%s.json" % ("native" if rendered else "headless")
	...
	if game:
		...
		game.queue_free()
		await process_frame
	quit(0 if results.all(func(result): return result.finished and not result.crashed) else 1)
