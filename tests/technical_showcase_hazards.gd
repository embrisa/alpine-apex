...
func run() -> void:
	...
	for speed in [40.0,55.0,70.0,95.0]:
		for x in [50.0,-222.0]:
			...
			results.append({"fixture":"drop" if x==50 else "snow_bypass","entry_kmh":speed,"airtime_s":sim.total_airtime,"landed":landed,"crash":sim.crash_reason,"position":str(sim.position),"peak_normal_impact_ms":peak_impact})
	for side in [-1,1]:
		var sim = SkiSimulation.new(preload("res://config/ski_default.tres").duplicate(true))
