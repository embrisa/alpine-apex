...
func run() -> void:
	...
	if field==null: printerr("V16_PROBE_FAILED"); quit(1); return
	...
	preload("res://tests/test_report.gd").write("res://artifacts/generation_v16/"+name+".json",JSON.stringify(report,"\t"))
	print("V16_PROBE_READY ",JSON.stringify(report))
	quit()
