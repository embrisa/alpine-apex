...
func run() -> void:
	...
	if field==null: printerr("V17_PROBE_FAILED"); quit(1); return
	...
	preload("res://tests/test_report.gd").write("res://artifacts/generation_v17/"+name+".json",JSON.stringify(report,"\t"))
	print("V17_PROBE_READY ",JSON.stringify(report))
	quit()
