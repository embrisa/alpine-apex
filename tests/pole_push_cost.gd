...
## One excluded traversal then one Off/On pair; source/engine metadata only.
var destination = ""
var arms: Array = []
...
func run() -> void:
	...
	if capture:
		...
		for arm in 3:
			game.skier.pole_carry_clearance_enabled=arm!=1
			...
			report={"arm":["warmup","off","on"][arm],"excluded":arm==0,"clearance_enabled":arm!=1}
			samples={"frame_ms":[],"render_cpu_ms":[],"gpu_ms":[],"draw_calls":[],"primitives":[]}
			unfocused_frames=0; previous_frame=0; powered_frames=0; clearance_frames=0; contact_cost=[]; pose_cost=[]
			...
			report.powered_render_frames=powered_frames; report.clearance_render_frames=clearance_frames
			...
			check(powered_frames>20,"Real pole thrust participates in every arm")
			check(arm==1 or clearance_frames>20,"Actual clearance correction participates when enabled")
		...
		report={"capture":false,"performance_evidence":failures.is_empty(),"setup_seconds":setup_seconds,"arms":arms,
			...
			"scope":"Local production mixed fixture:6s ordinary forward input, start from rest; excluded warmup then clearance Off/On. Not whole-mountain FPS or total propulsion feature isolation."}
	output=destination
