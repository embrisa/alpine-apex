...
func run() -> void:
	...
	preload("res://tests/test_report.gd").write("res://artifacts/generation_v17/cancellation.json",JSON.stringify(report,"\t"))
	print("V17_CANCELLATION ",JSON.stringify(report)); quit(0 if failures.is_empty() else 1)
