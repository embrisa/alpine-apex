...
const LegacyCache = preload("res://scripts/world/mountain_cache_v12.gd")
const DensityChecks = preload("res://tests/density_contract.gd")
...
func run() -> void:
	DirAccess.make_dir_recursive_absolute("res://artifacts/alpine_v13")
	check(Definition.parse_seed("42 / v13").version==13,"Archived v13 seeds remain explicit and reconstructible")
	check(Definition.parse_seed("849205174 / v13").version==13,"Explicit v13 seed sharing")
	var seeds = [849205174] if not "--all-seeds" in OS.get_cmdline_user_args() else [849205174,0,42,2147483647]
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--seed="): seeds = [clampi(int(arg.get_slice("=",1)),0,2147483647)]
	for seed_value in seeds:
		print("ALPINE_V13_BASELINE ",seed_value)
		var legacy=LegacyCache.generate(seed_value)
		var baseline=DensityChecks.population(legacy)
		baseline.height_sha256=legacy.height_checksum
		baseline.obstacle_sha256=legacy.obstacle_checksum
		baseline.exposure_sha256=Terrain._digest(legacy.exposure_image.get_data())
		legacy=null
		print("ALPINE_V13_GENERATE ",seed_value)
		var field = Cache.generate(seed_value)
		print("ALPINE_V13_BAKED ",seed_value," ms=",field.generation_ms)
		field.build_material_map()
		var report = Probe.measure(field)
		report.generation_stages=field.generation_stages.duplicate()
		report.population=DensityChecks.population(field)
		report.baseline=baseline
		check(field.height_checksum==baseline.height_sha256,"V13 preserves exact v12 support heights")
		check(Terrain._digest(field.exposure_image.get_data())==baseline.exposure_sha256,"V13 preserves v12 snow and contact material")
		check(field.obstacle_checksum!=baseline.obstacle_sha256,"V13 has distinct obstacle identity")
		check(DensityChecks.spacing(field),"All trunks retain 4.2 m minimum spacing")
		check(DensityChecks.drops(field),"New solids preserve intended drop entry and landing envelopes")
		for i in 6:
			check(report.population.faces[i].tree_cells>baseline.faces[i].tree_cells*1.8,"Face %d expands occupied woodland area" % i)
			check(report.population.faces[i].rock_cells>baseline.faces[i].rock_cells*1.8,"Face %d expands occupied mineral area" % i)
		check(report.minerals.categories.get("medium",0)>baseline.categories.get("medium",0)*3,"More substantial medium obstacles")
		check(report.minerals.categories.get("large",0)>baseline.categories.get("large",0)*2,"More visible large formations")
		print("ALPINE_V13_COVERAGE ",report.counts)
		...
		check(field.obstacles.size()<400000 and field.obstacles.size()>150000,"Seed %d stays within the dense regional tree budget" % seed_value)
		check(field.geology.placements.size()>14000 and field.geology.placements.size()<20000,"Seed %d has bounded clustered mineral detail" % seed_value)
		...
		for face in field.faces:
			...
			var widths: Array = stats.reachable_columns.slice(20*stats.refinement)
			...
			check(separation>=500 and widths.min()>=1 and ordered_widths[ordered_widths.size()/2]*stats.across_m>=144,"Seed %d face %d offers distributed reachable snow with local constrictions" % [seed_value,face.index])
			check(stats.merge_nodes>100,"Seed %d face %d has branching and rejoining connections" % [seed_value,face.index])
			check(DensityChecks.swept_routes_clear(field,face,paths),"Seed %d face %d sampled alternatives pass actual swept scenery collision" % [seed_value,face.index])
			var treeline = 0
			var dense = 0
			var sparse = 0
			var open = 0
			...
			for stand in face.stands:
				...
				var trunks = 0
				for id in field.nearby_obstacle_indices(Vector3(centre.x,0,centre.y),60.0):
			...
			stats.minimum_reachable_width_m = widths.min()*stats.across_m
			stats.median_reachable_width_m = ordered_widths[ordered_widths.size()/2]*stats.across_m
			stats.paths = paths.map(func(path): return path.map(func(p): return [p.x,p.y]))
			surveys.append(stats)
			print("ALPINE_V13_FACE ",seed_value," ",face.index," exits=",separation," width=",stats.minimum_reachable_width_m)
		report.surveys = surveys
		report.route_sampling={"across_m":Survey.X_STEP,"downhill_m":Survey.STEP,"node_clearance_m":Survey.NODE_CLEARANCE,"edge_clearance_m":Survey.EDGE_CLEARANCE,"solver_trunk_expansion_m":.35}
		var recipe = Definition.from_field(field,"Alpine regions")
		check(Definition.decode(recipe.share_text()).error.is_empty(),"V13 recipe round trip")
		...
		if "--repeat" in OS.get_cmdline_user_args():
			...
			report.independent_repeat_ms=field.generation_ms
		field = null
