...
func run() -> void:
	...
	for repetition in repetitions:
		...
		if version==14: work = load("res://scripts/world/mountain_cache_v14.gd").generate.bind(849205174)
			...
			work = load("res://scripts/world/mountain_cache_v15.gd").generate.bind(849205174,settings)
		worker.start(work)
		...
		if version==15 and preset_index==1 and repetition==0:
			await game.mountain_library.open()
			var library = game.mountain_library
			library.advanced_toggle.button_pressed = false
			await process_frame; await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://artifacts/generation_v15/controls_standard.png")
			...
			await process_frame; await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://artifacts/generation_v15/controls_custom.png")
			var scroll = library.tabs.get_current_tab_control()
			scroll.ensure_control_visible(library.estimates_label)
			await process_frame; await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://artifacts/generation_v15/controls_estimates.png")
		...
		if repetition==0:
			await process_frame; await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://artifacts/generation_v15/v%d_p%d%s_summit.png" % [version,preset_index,suffix])
		var dense: Array = []
