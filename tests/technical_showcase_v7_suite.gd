...
const OUTPUT = "res://artifacts/technical_showcase_v7"
var failures: Array = []
var checks = 0
...
func run() -> void:
	...
	var field = Definition.generate(849205174,7)
	...
	race.title = "V7 drainage compatibility"
	...
	check(shared_race.has("race") and shared_race.race.mountain==recipe.to_reference() and shared_race.race.identity()==race.identity(),"Shared races round-trip the exact v7 mountain and race identity")
	var invalid_reference = recipe.to_reference()
	invalid_reference.seed = 42
	invalid_reference.scenery_seed = 42
	check("only seed" in Definition.reference_error(invalid_reference),"Unsupported v7 seeds are rejected in imported references too")
