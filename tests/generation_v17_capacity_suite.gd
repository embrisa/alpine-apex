...
func run() -> void:
	...
	var path = "res://artifacts/generation_v17/synthetic_capacity.physical"
	...
	if DisplayServer.get_name()!="headless":
		...
		var scenery_path = "res://artifacts/generation_v17/synthetic_capacity.scenery"
		start = Time.get_ticks_usec()
	...
	preload("res://tests/test_report.gd").write("res://artifacts/generation_v17/capacity_"+("headless" if DisplayServer.get_name()=="headless" else "native")+".json",JSON.stringify(report,"\t"))
	print("V17_CAPACITY ",JSON.stringify(report)); quit(0 if failures.is_empty() else 1)
