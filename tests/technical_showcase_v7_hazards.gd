...
func run() -> void:
	DirAccess.make_dir_recursive_absolute("res://artifacts/technical_showcase_v7")
	field = Definition.generate(849205174,7)
	...
	FileAccess.open("res://artifacts/technical_showcase_v7/hazards.json",FileAccess.WRITE).store_string(JSON.stringify({"checks":checks,"failures":failures,"results":results},"\t"))
