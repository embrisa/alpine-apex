...
func replay():
	...
	check(Replay.VERSION==5 and Replay.INPUT_WIDTH==8 and restored!=null,"Replay v5 accepts eight-field tick records")
	if restored: check(absf(restored.input_at(4).air_tilt+.7)<.000001,"Replay round trip retains negative normal pitch")
	var invalid = data.duplicate(true)
	var bad_inputs: PackedFloat32Array = Marshalls.base64_to_raw(invalid.inputs_f32).to_float32_array()
	bad_inputs[7] = 1.1; invalid.inputs_f32 = Marshalls.raw_to_base64(bad_inputs.to_byte_array())
	check(Replay.decode(invalid,identity,.1)==null,"Replay rejects out-of-range normal pitch")
	bad_inputs[7] = NAN; invalid.inputs_f32 = Marshalls.raw_to_base64(bad_inputs.to_byte_array())
