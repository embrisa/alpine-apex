extends SceneTree
## Isolated SSIL comparison; existing sky lighting and SSAO stay fixed.
## Manual simulation steps keep paired views identical and out of records.
var game
var captures: Array = []
var failures: Array[String] = []
const OUTPUT = "res://artifacts/ssil_lighting"
...
func run() -> void:
	if DisplayServer.get_name() == "headless":
		printerr("SSIL inspection requires a rendered viewport")
		quit(1)
		return
	...
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--ssil-benchmark="):
			if not "--autoplay" in OS.get_cmdline_user_args():
				printerr("SSIL benchmark requires --autoplay")
				quit(1)
				return
			game.world.environment.ssil_enabled = arg.get_slice("=",1)=="on"
			print("SSIL_BENCHMARK enabled=",game.world.environment.ssil_enabled)
			return
	await process_frame
	...
	for frame in range(120):
		...
		await process_frame
		if frame in [30,60,90,119]: await capture("moving_%03d" % frame,0)
	for level in [0,1,2,0]:
		game.set_graphics_quality(level)
		game.world.update_weather(game.weather.state,0.0,false)
		if game.world.environment.ssil_enabled != (level==2):
			failures.append("SSIL quality switching failed for tier %d" % level)
	...
	print("SSIL_LIGHTING_RESULTS ",JSON.stringify({"captures":captures.size(),"failures":failures}))
...
func capture(label: String, settle_frames: int = 30) -> void:
	...
	captures.append({"label":label,"speed_kmh":game.sim.speed_kmh(),"eligible":game.session.eligible,"ssil":game.world.environment.ssil_enabled})
