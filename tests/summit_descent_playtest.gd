...
func run() -> void:
	...
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--heading="): chosen_heading = deg_to_rad(float(arg.get_slice("=",1)))
	...
	var results: Array = []
	var sides = [0.0,PI/4,PI/2,3*PI/4,PI,-3*PI/4,-PI/2,-PI/4] if "--all-faces" in OS.get_cmdline_user_args() else [chosen_heading]
	for side in sides:
		var sim = game.sim if rendered else Simulation.new(preload("res://config/ski_default.tres").duplicate(true))
		sim.reset(field.launch_point(side),side)
		if rendered: game.summit_ready = false
		sim.prime_contacts(field)
		var input = RiderInput.new()
		var target_heading = side
		var ticks = 0
		...
		for i in range(24000):
			...
			# The test rider brakes for the lower forest; terrain never caps speed.
			if Vector2(sim.position.x,sim.position.z).length()>2100:
				input.brake = maxf(input.brake,clampf((sim.speed_kmh()-100)/20,0,1))
			input.tuck = .75
			if rendered:
				game.intent = input
			...
			if i%1200==0:
				...
				if rendered and capture_enabled:
					await RenderingServer.frame_post_draw
					root.get_texture().get_image().save_png("res://artifacts/summit_mountain/descent_%d.png" % i)
			if rendered and capture_enabled and not air_captured and sim.total_airtime>.12:
				air_captured = true
				await RenderingServer.frame_post_draw
				root.get_texture().get_image().save_png("res://artifacts/summit_mountain/descent_air.png")
			if sim.crashed or field.reached_base(sim.position): break
		recording = false
		results.append({"heading_degrees":rad_to_deg(side),"finished":field.reached_base(sim.position),"crashed":sim.crashed,"reason":sim.crash_reason,
	...
	var output = {"seed":seed_number,"version":version_number,"pilot_speed_limit_kmh":speed_limit,"pilot_tuck":.75,"forest_braking_target_kmh":100,"height_sha256":field.height_checksum,"obstacle_sha256":field.obstacle_checksum,"results":results,"rendered":rendered,"actual_pixels":[actual_pixels.x,actual_pixels.y],"capture_overhead_included":capture_enabled,
	...
	if version_number==4 and speed_limit==0:
		passed = passed and results.all(func(result): return result.peak_kmh>140 and result.sim_seconds<200)
...
func pilot_heading(sim, side: float) -> float:
	...
	if not sim.grounded: return sim.heading
	var lookahead = clampf(sim.velocity.length()*3.2,25,150)
	var best = preferred
	var best_cost = INF
	for j in range(-8,9):
		...
		for k in range(1,7):
			...
			for lateral in [-6.0,0.0,6.0]:
				if not field.sweep_obstacle(last+Vector3(direction.z,0,-direction.x)*lateral,probe+Vector3(direction.z,0,-direction.x)*lateral).is_empty(): cost += 35.0
