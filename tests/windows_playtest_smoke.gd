...
func run() -> void:
	...
	if not require(FileAccess.file_exists(OS.get_executable_path().get_base_dir().path_join("data/default_mountain_v18.physical")),"Bundled bake missing"): return
	if not require(not ProjectSettings.has_setting("autoload/MCPRuntimeServer"),"Development autoload was not stripped"): return
	if not require(not cache.Archive.read(OS.get_executable_path().get_base_dir().path_join("data/default_mountain_v18.physical"),cache.cache_key(849205174)).is_empty(),"Bundled bake does not match the exported engine and source data"): return
	started_ms = Time.get_ticks_msec()
	...
	if not require(game.field.GENERATOR_VERSION==18 and game.effects.wind.available,"World version or native audio incorrect"): return
