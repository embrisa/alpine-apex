...
func run() -> void:
	...
	var cache = load("res://scripts/world/mountain_cache_v17.gd")
	if not require(not FileAccess.file_exists(cache.path_for(849205174)),"Test requires an empty user mountain cache"): return
	if not require(FileAccess.file_exists(OS.get_executable_path().get_base_dir().path_join("data/default_mountain_v17.physical")),"Bundled bake missing"): return
	if not require(not ProjectSettings.has_setting("autoload/MCPRuntimeServer"),"Development autoload was not stripped"): return
	if not require(not cache.Archive.read(OS.get_executable_path().get_base_dir().path_join("data/default_mountain_v17.physical"),cache.cache_key(849205174)).is_empty(),"Bundled bake does not match the exported engine and source data"): return
	started_ms = Time.get_ticks_msec()
	...
	if not require(game.field.GENERATOR_VERSION==17 and game.effects.wind.available,"World version or native audio incorrect"): return
