...
func _warning_loop() -> void:
	...
	check(not recovery.hit(10.5,10.5,"LANDING",tuning) and absf(recovery.reserve-.65)<.00001,"Reference rough landing spends 35 percent of the bar")
	check(not recovery.hit(10.5,10.5,"SECOND SKI",tuning) and absf(recovery.reserve-.65)<.00001,"Two ski contacts do not charge twice for one landing")
	for i in range(24): recovery.step(DT,true,tuning)
	check(not recovery.hit(10.5,10.5,"RECONTACT",tuning) and absf(recovery.reserve-.65)<.00001,"Immediate recontact remains part of the same impact")
	for i in range(24): recovery.step(DT,true,tuning)
	check(not recovery.hit(3.2,10.5,"SMALL HOP",tuning) and absf(recovery.reserve-.65)<.00001,"Small hops do not spend reserve or restart the delay")
	check(not recovery.hit(10.5,10.5,"SECOND LANDING",tuning) and absf(recovery.reserve-.30)<.00001,"Second ordinary rough landing leaves reserve instead of enforcing a two-hit rule")
	for i in range(48): recovery.step(DT,true,tuning)
	check(recovery.hit(10.5,10.5,"THIRD LANDING",tuning) and recovery.reserve==0.0,"Accumulated rough landings crash exactly when the bar is empty")
	recovery.reset()
	recovery.hit(10.5,10.5,"LANDING",tuning)
	for i in range(120): recovery.step(DT,true,tuning)
	check(absf(recovery.reserve-.65)<.00001,"Recovery waits for smooth riding instead of immediately refunding the impact")
	for i in range(150): recovery.step(DT,true,tuning)
	check(recovery.reserve>.80 and recovery.reserve<.84,"The bar refills gradually after the delay")
	...
	check(absf(recovery.reserve-.65)<.00001,"Free flight cannot refill the impact bar")
	recovery.reset()
	check(not recovery.hit(50.0,10.5,"VERY HARD LANDING",tuning) and absf(recovery.reserve-.25)<.00001,"Single-impact damage is capped at 75 percent for forgiveness")
	check(not recovery.hit(100.0,10.5,"SAME CONTACT CLUSTER",tuning) and absf(recovery.reserve-.25)<.00001,"A stronger probe in the same event cannot stack beyond the event cap")
	var reserves: Array = []
...
func _landings() -> void:
	...
	_drop(sim,8.0,plane)
	check(not sim.crashed and sim.impacts.reserve>0.0,"A second survivable landing spends reserve rather than forcing a fall")
	for i in range(48): sim.step(DT,RiderInput.new(),plane)
	_drop(sim,20.0,plane)
	check(sim.crashed and sim.impacts.reserve==0.0 and sim.crash_reason=="IMPACT LIMIT / HARD LANDING","Actual accumulated landings fall when impact reserve reaches zero")
	sim.reset(Vector3.ZERO)
