...
func run() -> void:
	var report = {"engine":Engine.get_version_info(),"unranked":true,"version":17,"seed":849205174,"physical_source_sha256":Cache.Sources.signature(),"settings":Cache.Settings.preset(),"runs":[]}
	var failed = false
	for repetition in 4:
		...
		preload("res://tests/test_report.gd").write("res://artifacts/generation_v17/v17_baseline.json",JSON.stringify(report,"\t"))
		print("V17_PROFILE_RUN ",JSON.stringify(row))
	print("V17_PROFILE_COMPLETE failed=",failed)
